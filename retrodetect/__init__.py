...
def ensemblegetshift(imgA, imgB, searchbox=100, step=8, searchblocksize=50, ensemblesizesqrt=3):
    ...
    for i, start in enumerate(starts):
        shifts[i] = getshift(imgA, imgB, step=step, searchbox=searchbox, start=start,
                             end=start + np.array([searchblocksize, searchblocksize]))
...
def old_getblockmaxedimage(img, blocksize=70, offset=2):
    ...
    for x, blockx in enumerate(range(0, img.shape[0] - 2 * blocksize * offset, blocksize)):
        for y, blocky in enumerate(range(0, img.shape[1] - 2 * blocksize * offset, blocksize)):
            out_img[blockx + (blocksize * offset):blockx + blocksize + (blocksize * offset),
            blocky + (blocksize * offset):blocky + blocksize + (blocksize * offset)] = max_img[x, y]
    return out_img
...
def detectcontact(photolist, n, savesize=20, delsize=15, thresholds=[9, 0.75, 6], historysize=10, blocksize=10,
    ...
    for i, s in enumerate(sets):
        ...
        for s_nf in s['noflash']:
            if this_set:
                intertime = time()
                diff = detect(s['flash'][0]['img'], s_nf['img'],
                              blocksize=blocksize)  # for the current search image we dilate
    ...
    for i in range(Npatches):
        ...
        patch = img[y - savesize + imgcorrection:y + savesize + imgcorrection,
                x - savesize + imgcorrection:x + savesize + imgcorrection].astype(np.float32)
        ...
        if model is not None:
            outersurround = max(patch[16, 20], patch[20, 16], patch[24, 20], patch[20, 24], patch[16, 16],
                                patch[16, 24], patch[24, 16], patch[24, 24])
            innersurround = max(patch[18, 20], patch[20, 18], patch[22, 20], patch[20, 22], patch[18, 18],
                                patch[18, 22], patch[22, 18], patch[22, 22])
        ...
        contact.append({'x': x + imgcorrection, 'y': y + imgcorrection, 'patch': patch, 'searchpatch': searchpatch,
                        'mean': int(mean), 'centre': int(centre), 'innersurround': int(innersurround),
                        'outersurround': int(outersurround), 'searchmax': int(searchmax), 'centremax': int(centremax),
                        'confident': confident, 'prediction': pred[0][0]})
